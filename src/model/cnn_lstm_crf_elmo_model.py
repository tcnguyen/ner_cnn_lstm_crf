...
class CharCNNLSTMCRFElmoModel(BaseModel):

    def __init__(self):
        super(CharCNNLSTMCRFElmoModel, self).__init__()
        self.features_generator = ElmoFeaturesGenerator()
    ...
    def add_placeholders(self):
        ...
        self.words = tf.placeholder(tf.string, shape=[None, None],
                                    name="words")
    ...
    def get_feed_dict(self, features, labels=None, lr=None, dropout=None):
        ...
        char_ids, words = zip(*features)
        words, sequence_lengths = pad_sequences(words, pad_tok="")
    ...
    def add_word_embeddings_op(self):
        ...
        with tf.variable_scope("words"):
            word_embeddings = self.elmo(
                inputs={
                    "tokens": self.words,
                    "sequence_len": self.sequence_lengths
    ...
    def add_logits_op(self):
        ...
        with tf.variable_scope("bi-lstm"):
            ...
            cell_fw = tf.nn.rnn_cell.DropoutWrapper(
                cell_fw, output_keep_prob=self.dropout)

            cell_bw = tf.nn.rnn_cell.LSTMCell(Config.hidden_size_lstm)
            cell_bw = tf.nn.rnn_cell.DropoutWrapper(
                cell_bw, output_keep_prob=self.dropout)
    ...
    def evaluate(self, dataset, batch_size=Config.batch_size):
        ...
        for features, label_ids in self.features_generator.minibatches(dataset, batch_size):
            ...
            for tag_ids in pred_ids:
                preds.extend([self.features_generator.idx_to_tag[idx]
                              for idx in tag_ids])
            for tag_ids in label_ids:
                labels.extend([self.features_generator.idx_to_tag[idx]
                               for idx in tag_ids])

        precision, recall, f1 = conlleval.evaluate(
            true_seqs=labels, pred_seqs=preds, verbose=False)
